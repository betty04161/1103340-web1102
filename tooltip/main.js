...
function setupCanvas(barChartData, movieClean) {
  ...
  const barchart_width =
    svg_width - (barchart_margin.left + barchart_margin.right);
  const barchart_height =
    svg_height - (barchart_margin.top + barchart_margin.bottom);
  const this_svg = d3
  ...
  function mouseover(e) {
    ...
    tip
      .select("h4")
      .html(`${thisBarData.tagline}, ${thisBarData.runtime} min.`);
    d3.select(".tip-body")
      .selectAll("p")
      .data(bodyData)
      .join("p")
